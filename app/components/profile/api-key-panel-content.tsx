...
export function ApiKeyPanelContent() {
  ...
  const toggleRevealApiKey = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok || !data.key) throw new Error(data.error || t("revealFailed"));



      const apiKey = data.key;

      setVisibleKeys((current) => ({ ...current, [id]: apiKey }));
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <div className="font-mono text-xs text-muted-foreground">
                  ...
                  {visibleKeys[key.id] || (key.hasDisplayKey ? t("hiddenKey") : t("legacyKeyUnavailable"))}
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!key.hasDisplayKey}

                  title={visibleKeys[key.id] ? t("hideKey") : t("revealKey")}

                  onClick={() => toggleRevealApiKey(key.id)}
                ...
                <Button
                  ...
                  disabled={!visibleKeys[key.id]}

                  title={t("copyKey")}

                  onClick={() => visibleKeys[key.id] && copyToClipboard(visibleKeys[key.id])}
