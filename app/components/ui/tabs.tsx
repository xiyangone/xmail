...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "theme-surface-tabs-list inline-flex h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.List
      ref={combinedRef}
      className={cn(
        "relative flex w-full rounded-xl p-1 h-auto",
        className
      )}
      {...props}
    >
      <div 
        className="surface-tabs-thumb absolute top-1 bottom-1 rounded-lg transition-all duration-300 ease-in-out"
        style={{
          width: tabWidth,
          left: slidePosition
        }}
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ref={ref}
    className={cn(
      "relative z-10 flex-1 h-8 gap-2 flex items-center justify-center rounded-lg px-3 py-2 text-sm font-medium transition-colors duration-200 data-[state=active]:bg-transparent data-[state=active]:text-primary-foreground data-[state=inactive]:text-foreground/70 data-[state=inactive]:hover:text-foreground ring-offset-background focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
