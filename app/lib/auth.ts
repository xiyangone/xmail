...
import { Permission, ROLES, Role } from "./permissions";
...
import { ensureRoleDefaultPermissions, getUserPermissionSnapshot, userHasPermission } from "./permission-service";

const AVATAR_COLORS = [
  '#2196F3', '#009688', '#9C27B0', '#F44336', '#673AB7',
  '#3F51B5', '#4CAF50', '#FF5722', '#795548', '#607D8B',
];

function generateAvatarUrl(name: string): string {
  const initial = name[0]?.toUpperCase() ?? '?';
  const colorIndex = Array.from(name).reduce((acc, char) => acc + char.charCodeAt(0), 0) % AVATAR_COLORS.length;
  const backgroundColor = AVATAR_COLORS[colorIndex];
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="100" height="100">
    <rect width="100" height="100" fill="${backgroundColor}"/>
    <text x="50%" y="50%" fill="white" font-family="system-ui, -apple-system, sans-serif"
      font-size="45" font-weight="500" text-anchor="middle" dominant-baseline="central">
      ${initial}
    </text>
  </svg>`;
  const base64 = Buffer.from(new TextEncoder().encode(svg)).toString('base64');
...
async function findOrCreateRole(db: Db, roleName: Role) {
  ...
  await ensureRoleDefaultPermissions(db, role.id, roleName);
