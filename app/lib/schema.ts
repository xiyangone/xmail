...
export const permissions = sqliteTable(
  ...
  (table) => ({
    ...
    keyIdx: uniqueIndex("permission_key_unique").on(table.key),
...
export const rolePermissions = sqliteTable(
  ...
  (table) => ({
    ...
    permissionKeyIdx: index("role_permission_permission_key_idx").on(table.permissionKey),
...
export const routePolicies = sqliteTable(
...

  {
    ...
    access: text("access").notNull().default("permission"),


    requiredPermissions: text("required_permissions"),


    allowApiKey: integer("allow_api_key", { mode: "boolean" }).notNull().default(false),


    allowInternal: integer("allow_internal", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    ...
    enabledPriorityIdx: index("route_policy_enabled_priority_idx").on(table.enabled, table.priority),


    pathPatternIdx: index("route_policy_path_pattern_idx").on(table.pathPattern),
...
export const workerRuns = sqliteTable(
  ...
  (table) => ({
    ...
    workerStartedAtIdx: index("worker_run_worker_started_at_idx").on(table.workerName, table.startedAt),


    statusIdx: index("worker_run_status_idx").on(table.status),
...
export const emailReceiverLogs = sqliteTable(
...

  {
    ...
    hasWebhook: integer("has_webhook", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    ...
    createdAtIdx: index("email_receiver_log_created_at_idx").on(table.createdAt),


    statusIdx: index("email_receiver_log_status_idx").on(table.status),


    recipientIdx: index("email_receiver_log_recipient_idx").on(table.recipient),
...
export const apiKeyScopes = sqliteTable(
  ...
  (table) => ({
    ...
    pk: primaryKey({ columns: [table.apiKeyId, table.permissionKey] }),

    permissionKeyIdx: index("api_key_scope_permission_key_idx").on(table.permissionKey),
